...
class SSLCertificateGenerator:
    ...
    def generate_certificate(self, domain, email):
        ...
        try:
            ...
            # 创建带有超时配置的ACME客户端
            net = client.ClientNetwork(jwk, user_agent='ssl-cert-generator/1.0', timeout=120)
            ...
            # 分离证书和CA证书
            cert_parts = fullchain_pem.split('\n\n')
            certificate = cert_parts[0] + '\n'
            ca_certificate = '\n\n'.join(cert_parts[1:]) if len(cert_parts) > 1 else ''
